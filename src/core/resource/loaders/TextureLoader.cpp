...
TextureLoader::TextureLoader(const aiScene* pScene,
...
TextureResource* TextureLoader::createEmbeddedTexture()
{
    ...
    {
        tTextureData = stbi_load_from_memory(reinterpret_cast<unsigned char*>(tTexture->pcData),
                                             tTexture->mWidth,
                                             &tWidth,
                                             &tHeight,
                                             &tChannelCount,
    ...
    {
        tTextureData = stbi_load_from_memory(reinterpret_cast<unsigned char*>(tTexture->pcData),
                                             tTexture->mWidth * tTexture->mHeight,
                                             &tWidth,
                                             &tHeight,
                                             &tChannelCount,
                                             0);
...
TextureResource* TextureLoader::createReferencedTexture()
{
    ...
    unsigned char* tTextureData = stbi_load(tTexturePath.c_str(),
                                            &tWidth,
                                            &tHeight,
                                            &tChannelCount,
                                            0);
...
{
    const std::string& tTextureFileName = FileManager::getInstance()->getFileNameFromPath(mTexturePath);

    createPossibleTextureLocations();

    for (unsigned int i = 0; i < mPossibleTextureLocations.size(); i++)
    {
        if (FileManager::getInstance()->isFileExist(mPossibleTextureLocations[i],
                                                    tTextureFileName))
        {
            return FileManager::getInstance()->createPath(mPossibleTextureLocations[i],
                                                          tTextureFileName);
        }
    }
    ...
    std::ranges::replace_if(tTextureFileNameSpacesChangedWithUnderscore.begin(),
    ...
    {
        if (FileManager::getInstance()->isFileExist(mPossibleTextureLocations[i],
                                                    tTextureFileNameSpacesChangedWithUnderscore))
        {
            return FileManager::getInstance()->createPath(mPossibleTextureLocations[i],
                                                          tTextureFileNameSpacesChangedWithUnderscore);
        }
