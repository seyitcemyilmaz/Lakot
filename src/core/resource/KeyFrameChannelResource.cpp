...
int KeyFrameChannelResource::getRotationIndex(double pAnimationTime) const      ///TODO: "- 1" may cause some problems, it should be checked.
{
    for (int tIndex = 0; tIndex < mKeyFrameRotations.size() - 1; tIndex++)
    {
...
glm::mat4 KeyFrameChannelResource::interpolatePosition(double pAnimationTime) const
{
    ...
    double tScaleFactor = getScaleFactor(mKeyFramePositions[tP0Index]->timeStamp,
                                         mKeyFramePositions[tP1Index]->timeStamp,
                                         pAnimationTime);

    glm::vec3 tFinalPosition = glm::mix(mKeyFramePositions[tP0Index]->position,
                                        mKeyFramePositions[tP1Index]->position,
                                        tScaleFactor);
...
glm::mat4 KeyFrameChannelResource::interpolateRotation(double pAnimationTime) const
{
    ...
    {
        auto tRotation = glm::normalize(mKeyFrameRotations[0]->rotation);
    ...
    if (tP0Index == -1)
    {
        return glm::toMat4(glm::normalize(mKeyFrameRotations[getKeyFrameRotationsCount() - 1]->rotation));
    }

    double tScaleFactor = getScaleFactor(mKeyFrameRotations[tP0Index]->timeStamp,
                                         mKeyFrameRotations[tP1Index]->timeStamp,
                                         pAnimationTime);

    glm::quat tFinalRotation = glm::slerp(mKeyFrameRotations[tP0Index]->rotation,
                                          mKeyFrameRotations[tP1Index]->rotation,
                                          static_cast<float>(tScaleFactor));
...
glm::mat4 KeyFrameChannelResource::interpolateScale(double pAnimationTime) const
{
    ...
    double tScaleFactor = getScaleFactor(mKeyFrameScales[tP0Index]->timeStamp,
                                         mKeyFrameScales[tP1Index]->timeStamp,
                                         pAnimationTime);

    glm::vec3 tFinalScale = glm::mix(mKeyFrameScales[tP0Index]->scale,
                                     mKeyFrameScales[tP1Index]->scale,
                                     tScaleFactor);
